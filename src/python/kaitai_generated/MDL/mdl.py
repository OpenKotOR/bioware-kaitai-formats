...
class Mdl(KaitaiStruct):
    ...
    class Controller(KaitaiStruct):
        ...
        def _read(self):
            self.type = KaitaiStream.resolve_enum(
                bioware_mdl_common.BiowareMdlCommon.ControllerType, self._io.read_u4le()
            )
    ...
    class MdlAnimationEntry(KaitaiStruct):
        ...
        def header(self):
            ...
            _pos = self._io.pos()
            self._io.seek(
                self._root.data_start + self._root.animation_offsets[self.anim_index]
            )
    ...
    class ModelHeader(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.model_type = KaitaiStream.resolve_enum(
                bioware_mdl_common.BiowareMdlCommon.ModelClassification,
                self._io.read_u1(),
            )
    ...
    class Node(KaitaiStruct):
        ...
        def _read(self):
            ...
            if self.header.node_type == 17:
                pass
                self.reference_sub_header = Mdl.ReferenceHeader(
            ...
            if self.header.node_type == 97:
                pass
                self.skinmesh_sub_header = Mdl.SkinmeshHeader(
                    self._io, self, self._root
                )

            if self.header.node_type == 161:
                pass
                self.animmesh_sub_header = Mdl.AnimmeshHeader(
                    self._io, self, self._root
                )

            if self.header.node_type == 289:
                pass
                self.danglymesh_sub_header = Mdl.DanglymeshHeader(
                    self._io, self, self._root
                )
            ...
            if self.header.node_type == 2081:
                pass
                self.lightsaber_sub_header = Mdl.LightsaberHeader(
                    self._io, self, self._root
                )
    ...
    def animations(self):
        ...
        if self.model_header.animation_count > 0:
            ...
            self._m_animations = []
            for i in range(self.model_header.animation_count):
                self._m_animations.append(
                    Mdl.MdlAnimationEntry(i, self._io, self, self._root)
                )
    ...
    def names_data(self):
        ...
        if self.model_header.name_offsets_count > 0:
            ...
            self._m_names_data = Mdl.NameStrings(
                _io__raw__m_names_data, self, self._root
            )
            self._io.seek(_pos)
