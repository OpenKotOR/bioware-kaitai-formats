...
class GffBts(KaitaiStruct):
    """**BTS** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
    ...
    FileType / restype id **2034** — see `bioware_type_ids::xoreos_file_type_id` enum member `bts`.
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(GffBts, self).__init__(_io)
