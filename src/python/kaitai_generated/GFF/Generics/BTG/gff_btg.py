...
class GffBtg(KaitaiStruct):
    """**BTG** resources are **GFF3** on disk (Aurora `GFF ` prefix + V3.x version). Wire layout is fully defined by
    ...
    FileType / restype id **2054** — see `bioware_type_ids::xoreos_file_type_id` enum member `btg`.
    ...
    def __init__(self, _io, _parent=None, _root=None):
        super(GffBtg, self).__init__(_io)
