...
class Key(KaitaiStruct):
    ...
    def _read(self):
        ...
        if not self.file_type == "KEY ":
            raise kaitaistruct.ValidationNotEqualError(
                "KEY ", self.file_type, self._io, "/seq/0"
            )
        self.file_version = (self._io.read_bytes(4)).decode("ASCII")
        if not ((self.file_version == "V1  ") or (self.file_version == "V1.1")):
            raise kaitaistruct.ValidationNotAnyOfError(
                self.file_version, self._io, "/seq/1"
            )
    ...
    class FileEntry(KaitaiStruct):
        ...
        def filename(self):
            ...
            self._m_filename = (self._io.read_bytes(self.filename_size)).decode("ASCII")
            self._io.seek(_pos)
    ...
    class FilenameTable(KaitaiStruct):
        ...
        def _read(self):
            self.filenames = (self._io.read_bytes_full()).decode("ASCII")
