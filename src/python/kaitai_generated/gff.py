...
class Gff(KaitaiStruct):
    ...
    **Pinned URLs and tool history:** `meta.xref` (alphabetical keys). Coverage matrix: `docs/XOREOS_FORMAT_COVERAGE.md`.
    ...
    .. seealso::
       xoreos — G2DA `kG2DAID` + `kVersion01` / `kVersion02` (pair with in-tree `gff4_g2da_*` enums) - https://github.com/xoreos/xoreos/blob/master/src/aurora/gdafile.cpp#L40-L42
    ...
    class FieldEntry(KaitaiStruct):
        ...
        def _read(self):
            self.field_type = KaitaiStream.resolve_enum(bioware_gff_common.BiowareGffCommon.GffFieldType, self._io.read_u4le())
        ...
        def is_complex_type(self):
            ...
            self._m_is_complex_type =  ((self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint64) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int64) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.double) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.string) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.resref) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.localized_string) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.binary) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.vector4) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.vector3)) 
        ...
        def is_list_type(self):
            ...
            self._m_is_list_type = self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.list
        ...
        def is_simple_type(self):
            ...
            **PyKotor `GFFBinaryReader`:** type **18 is not handled** after the float branch — see https://github.com/OpenKotOR/PyKotor/blob/master/Libraries/PyKotor/src/pykotor/resource/formats/gff/io_gff.py#L268-L273 (wire layout for 18 is still per wiki + this `.ksy`).
            ...
            self._m_is_simple_type =  ((self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint8) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int8) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint16) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int16) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint32) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int32) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.single) or (self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.str_ref)) 
        ...
        def is_struct_type(self):
            ...
            self._m_is_struct_type = self.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.struct
    ...
    class Gff4AfterAurora(KaitaiStruct):
        ...
        def _read(self):
            ...
            for i in range(self.num_struct_templates):
                self.struct_templates.append(Gff.Gff4StructTemplateHeader(self._io, self, self._root))
    ...
    class GffUnionFile(KaitaiStruct):
        ...
        def _read(self):
            self.aurora_magic = self._io.read_u4be()
            self.aurora_version = self._io.read_u4be()
            if  ((self.aurora_version != 1446260272) and (self.aurora_version != 1446260273)) :
                pass
                self.gff3 = Gff.Gff3AfterAurora(self._io, self, self._root)

            if  ((self.aurora_version == 1446260272) or (self.aurora_version == 1446260273)) :
                pass
                self.gff4 = Gff.Gff4AfterAurora(self.aurora_version, self._io, self, self._root)
    ...
    class ResolvedField(KaitaiStruct):
        ...
        def list_entry(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.list:
                pass
                _pos = self._io.pos()
        ...
        def value_binary(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.binary:
                pass
                _pos = self._io.pos()
        ...
        def value_double(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.double:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_int16(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int16:
                pass
                _pos = self._io.pos()
        ...
        def value_int32(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int32:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_int64(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int64:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_int8(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.int8:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_localized_string(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.localized_string:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_resref(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.resref:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_single(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.single:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_str_ref(self):
            ...
            **reone** implements `StrRef` as **`field_data`-relative** (`readStrRefFieldData`), not as an inline dword at +8: https://github.com/modawan/reone/blob/master/src/libs/resource/format/gffreader.cpp#L141-L143 — https://github.com/modawan/reone/blob/master/src/libs/resource/format/gffreader.cpp#L199-L204 (treat as cross-engine / cross-tool variance when porting assets).
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.str_ref:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_string(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.string:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_struct(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.struct:
                pass
        ...
        def value_uint16(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint16:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_uint32(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint32:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_uint64(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint64:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_uint8(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.uint8:
                pass
                _pos = self._io.pos()
                self._io.seek(self.field_entry_pos + 8)
        ...
        def value_vector3(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.vector3:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
        ...
        def value_vector4(self):
            ...
            if self.entry.field_type == bioware_gff_common.BiowareGffCommon.GffFieldType.vector4:
                pass
                _pos = self._io.pos()
                self._io.seek(self._root.file.gff3.header.field_data_offset + self.entry.data_or_offset)
